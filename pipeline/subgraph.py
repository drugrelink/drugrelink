...
def subgraph(path,graph,cutoff,pnumber,nnumber):
    df_features = pd.read_csv(path,sep ='\t',low_memory=False)
    ...
    for index,row in df_positive.iterrows():
        ...
        for path in nx.all_simple_paths(graph,source='Compound::'+row['compound_id'],target='Disease::'+row['disease_id'],cutoff=cutoff):
    ...
    for i,r in df_negative.iterrows():
        ...
        for path in nx.all_simple_paths(graph, source='Compound::'+r['compound_id'], target='Disease::'+r['disease_id'],cutoff=cutoff):
            subgraph_nodeslist=subgraph_nodeslist+path   
